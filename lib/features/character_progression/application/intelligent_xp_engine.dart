...
class IntelligentXPEngine {
  ...
  XPCalculationBreakdown calculateDetailedXP(Task task, CompletionContext context) {
    ...
    int finalBaseXP = (baseTimeXP * categoryMultiplier * difficultyMultiplier).round();
    finalBaseXP = finalBaseXP < 5 ? 5 : finalBaseXP; // Ensure minimum viable XP
    
    // Calculate all bonuses
    int streakBonus = context.currentStreak > 1 ? calculateStreakBonus(task, context.currentStreak) : 0;
    int perfectWeekBonus = context.perfectWeeksThisMonth > 0 ? calculatePerfectWeekBonus(task, context.perfectWeeksThisMonth) : 0;
    
    bool isMorningHabit = this.isMorningHabit(task);
    bool completedInMorning = isCompletedInMorning(context.completionTime);
    int morningBonus = (isMorningHabit && completedInMorning) ? (finalBaseXP * 0.1).round() : 0;
    
    // Calculate loot box bonus (chance for XP multiplier)
    LootBoxResult lootBoxResult = _calculateLootBoxBonus(task, finalBaseXP + streakBonus + perfectWeekBonus + morningBonus);
    
    int totalBonusXP = streakBonus + perfectWeekBonus + morningBonus + lootBoxResult.bonusXP;
