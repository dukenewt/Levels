...
class TalentTriggerService with FeatureFlagMixin {
  static final TalentTriggerService _instance = TalentTriggerService._internal();
  ...
  void _checkForTalentChoice() {
    ...
    if (hasLogDetailed) {
      debugPrint('TalentTrigger: Checking talent choice - Level: ${user?.level}, Needs choice: ${state.needsTalentChoice}');
    }
    ...
    if (state.needsTalentChoice && 
      ...
      if (hasLogDetailed) {
        debugPrint('TalentTrigger: Triggering talent choice for level ${state.talentChoiceLevel}');
      }
      
      // Create talent choice from controller state using existing system
      final talentChoice = UserTalents.getTalentChoice(state.talentChoiceLevel!);
  ...
  Future<void> _showTalentDialog(TalentChoice talentChoice) async {
    ...
    try {
      if (hasLogDetailed) {
        debugPrint('TalentTrigger: Showing talent dialog for level ${talentChoice.level}');
      }
