    ...
    try {
      ...
      if (key.isEmpty) {
        return Result.failure(
            ValidationException('Storage key cannot be empty'));
      }
    ...
    try {
      ...
      if (rawValue == null) {
        if (defaultValue != null) {
          ...
          return Result.failure(
              StorageException('No data found for key "$key"'));
        }
      }
      ...
      try {
        ...
        if (defaultValue != null) {
          debugPrint(
              'Warning: Failed to convert stored data for key "$key", using default value');
          return Result.success(defaultValue);
        } else {
          return Result.failure(StorageException(
...
class SecureTaskRepository {
  final SharedPreferences _prefs;
  static const String _tasksKey =
      'tasks_v1'; // Version your keys for future migrations
      ...
      try {
        ...
        for (int i = 0; i < decodedTasks.length; i++) {
          try {
            ...
            debugPrint(
                'Warning: Skipped corrupted task at index $i: $taskError');
        ...
        debugPrint(
            'Task parsing failed, attempting backup restore: $parseError');
  ...
  Future<Result<void>> saveTasks(List<Task> tasks) async {
    try {
      ...
      if (tasks.any((task) => task.id.isEmpty)) {
        return Result.failure(
            ValidationException('All tasks must have valid IDs'));
      }
      ...
      if (!success) {
        return Result.failure(
            StorageException('Failed to save tasks to storage'));
      }
      ...
      if (verification.isSuccess && verification.data!.length == tasks.length) {
        ...
        return Result.failure(
            StorageException('Task save verification failed'));
      }
    } catch (e, stackTrace) {
      final error = StorageException(
  ...
  Future<Result<void>> updateTask(Task task) async {
    ...
    if (existingIndex == -1) {
      return Result.failure(
          ValidationException('Task with ID ${task.id} not found'));
    }
  ...
  Future<Result<void>> deleteTask(String taskId) async {
    ...
    if (tasks.length == originalLength) {
      return Result.failure(
          ValidationException('Task with ID $taskId not found'));
    }
  ...
  Future<Result<Map<String, dynamic>>> getUserData() async {
    try {
      ...
      return Result.success(
          <String, dynamic>{}); // Return empty data as fallback
    }
  }
